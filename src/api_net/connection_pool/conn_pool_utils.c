...
int ap_net_recv(int sh, void *buf, int size, int non_blocking)
{
	...
	DEBUGMSG(0,("retval:%d, size:%d\r\n", retval, size));
	DEBUGMEM(0,("buf:", buf, retval));
	if ( retval < 0 )
...
int ap_net_send(int sh, void *buf, int size, int non_blocking)
{
    ...
    retval = fcntl(sh, F_GETFL);
	DEBUGMSG(0,("%s..., retval:%d\r\n", __FUNCTION__, retval));

	flags = fcntl(sh,F_GETFL,0);
	flags &= ~O_NONBLOCK;
	retval = fcntl(sh,F_SETFL,flags);

    if ( retval != -1 )
    {
    	flags = (non_blocking ? MSG_DONTWAIT : 0) | MSG_NOSIGNAL;
		DEBUGMSG(0,("-----flags:%d, sh:%d\r\n", flags, sh));
