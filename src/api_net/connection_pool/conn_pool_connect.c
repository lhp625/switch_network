...
static struct ap_net_connection_t *ap_net_conn_pool_do_connect(struct ap_net_conn_pool_t *pool, int conn_idx)
{
	...
	DEBUGMSG(1,("1 *****************************flags:[%d] ?= %d\n", 
		pool->flags, AP_NET_POOL_FLAGS_TCP));	
	//无论是TCP或UDP都绑定
    //if ( !bit_is_set(pool->flags, AP_NET_POOL_FLAGS_TCP) ) /* do manual bind() for UDP */
    {
		memset(&conn->local, 0, sizeof(conn->local));
    	conn->local.af = pool->local.af;
		conn->local.addr4.sin_port = pool->local.addr4.sin_port;
		//conn->local.sin_addr.s_addr = htonl(address4);
		
		DEBUGMSG(1,("2 *****************************, conn->fd:%d\n", conn->fd));
		// 设置套接字选项避免地址使用错误   
...
static int sanity_check(struct ap_net_conn_pool_t *pool, int port, int expire_in_ms, unsigned flags)
{
	...
	DEBUGMSG(1,("%s...\n", __FUNCTION__));
	DEBUGMSG(0,("expire_in_ms:%d\n", expire_in_ms));
    if(expire_in_ms < 0)
    	return -1;
	
	DEBUGMSG(1,("used_slots:%d, pool->max_connections:%d\n", pool->used_slots, pool->max_connections));
    ...
    for ( conn_idx = 0; conn_idx < pool->max_connections; ++conn_idx ) /* finding free slot */
        if ( ! (pool->conns[conn_idx].state & AP_NET_ST_CONNECTED) )
            break;
	DEBUGMSG(1,("conn_idx:%d, max_connections:%d\n", conn_idx, pool->max_connections));
